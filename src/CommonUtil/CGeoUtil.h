...
public:
     ...
     * @retval  false   失敗
    */
    bool GetInterseectionPt(const CVector3D &pos, const CVector3D &vec, CVector3D &crossPt);
...
#pragma region 幾何計算
    ...
    // 2つのベクトルのなす角(v1を基準とし、反時計回りを正とする符号付)
    static double SignedAngle(const CVector2D &v1, const CVector2D &v2);

    // 回転
    static CVector2D Rotate(
        const CVector2D &pt,        // 頂点
        const CVector2D &center,    // 回転時の中心点
        double dDeg);               // 回転角度deg

    // 回転
    static std::vector<CVector2D> Rotate(
        const std::vector<CVector2D> &pts,  // 頂点列
        const CVector2D &center,            // 回転時の中心点
        double dDeg);                       // 回転角度deg
