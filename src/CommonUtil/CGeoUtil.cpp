...
/*!
 * @brief 2つのベクトルのなす角(v1を基準とし、反時計回りを正とする符号付)
 * @param[in] v ベクトル
 * @return ベクトルのなす角(deg)
 * @note 0ベクトルの場合は、0を返却
*/
double CGeoUtil::SignedAngle(const CVector2D &v1, const CVector2D &v2)
{
    double dAngle = Angle(v1, v2);
    // 確度の向きを決めるためv1,2の外積ベクトルを使用する
    CVector3D v;
    OuterProduct(CVector3D(v1.x, v1.y, 0), CVector3D(v2.x, v2.y, 0), v);
    v.Normalize();
    if (v.z < 0)
        dAngle *= -1;
    return dAngle;
 ...
 * @brief 回転
 * @param[in] pt        頂点
 * @param[in] center    回転時の中心点
 * @param[in] dDeg      回転角度deg
 * @return 回転結果
*/
CVector2D CGeoUtil::Rotate(
    const CVector2D &pt,
    const CVector2D &center,
    double dDeg)
{
    double dRad = dDeg * _COEF_DEG_TO_RAD;
    CVector2D movePt = pt - center;
    double x = movePt.x * cos(dRad) - movePt.y * sin(dRad);
    double y = movePt.x * sin(dRad) + movePt.y * cos(dRad);
    CVector2D rotPt(x, y);
    rotPt = rotPt + center;
    return rotPt;
}
 ...
 * @return 回転結果
*/
std::vector<CVector2D> CGeoUtil::Rotate(
    const std::vector<CVector2D> &pts,
    const CVector2D &center,
    double dDeg)
{
    std::vector<CVector2D> dst;
    for (const auto &pt : pts)
    {
        CVector2D rotPt = Rotate(pt, center, dDeg);
        dst.push_back(rotPt);
    }
    return dst;
}
...
/*!
 * @brief  入力ベクトルに垂直なベクトルの取得
 * @param vec               入力ベクトル
 * @param vecVertical       出力ベクトル
 * @return サンプリング後のマルチポリライン
*/
bool CGeoUtil::GetVerticalVec(
    const CVector2D &vec,
...
/*!
 * @brief 2直線の交点算出
 * @param vec1          入力方向ベクトル1
 * @param pos1          入力ベクトル始点1
 * @param vec2          入力方向ベクトル2
 * @param pos2          入力ベクトル始点2
 * @param pos           交点
 * @param bOnLine1      交点がベクトル1上にあるか
 * @param bOnLine2      交点がベクトル2上にあるか
 * @param t             交点位置までのベクトル1のベクトル係数
 * @param s             交点位置までのベクトル2のベクトル係数
 * @return 処理結果
 * @retval true     成功
 * @retval false    失敗
*/
bool CGeoUtil::GetCrossPos(
    ...
    double &s)
{
    ...
    // vec1に垂直なベクトル
    CVector2D vecVertical;
    if (!GetVerticalVec(vec1, vecVertical))
        return false;
 ...
 * @param pos           入力ベクトル始点
 * @param crossPos      交点
 * @param bOnLine       交点がベクトル上にあるか
 * @param t             交点位置までのベクトルのベクトル係数
 * @param dist          交点位置までの距離
 * @return 処理結果
 * @retval true     成功
 * @retval false    失敗
*/
...
#pragma region 平面クラス
// 平面と直線の交点の算出
bool CPlane::GetInterseectionPt(const CVector3D &pos, const CVector3D &vec, CVector3D &crossPt)
{
    if (vec.Length() == 0)
        return false;

    double t = -(a * pos.x + b * pos.y + c * pos.z + d)
...
// 法線ベクトル(正規化済み)
CVector3D CPlane::NormalVec()
{
    CVector3D vec(a, b, c);
    vec.Normalize();
    return vec;
}
#pragma endregion 平面クラス
